...
constexpr wchar_t kWindowClass[] = L"TouchPlusRevivalStereoViewerRT";
...
void convert_yuy2_to_bgra_vertical_flip(const BYTE* input, DWORD length,
    ...
    // Historical Ractiv code performs cv::flip(frame, frame_flipped, 0)
    // before splitting the 1280x480 image. Reproduce that vertical flip here
    // while preserving the horizontal axis and left/right stereo identity.
...
void capture_thread(HWND hwnd) {
    try {
        ...
        std::cout << "[2/3] Opening native MJPEG -> YUY2 real-time stream\n";
        ...
        // Intentionally do NOT enable MF_SOURCE_READER_ENABLE_VIDEO_PROCESSING.
        // That legacy software YUV->RGB path is documented by Microsoft as not
        // optimized for real-time playback. Let Source Reader insert only the
        // decoder, ask for YUY2, then do the tiny color conversion ourselves.
        ...
        MFGetAttributeRatio(actual_type.Get(), MF_MT_FRAME_RATE,
                            &actual_fps_num, &actual_fps_den);
        std::cout << "Decoded output: " << actual_width << "x" << actual_height
                  << " @ " << (actual_fps_den ? static_cast<double>(actual_fps_num) / actual_fps_den : 0.0)
                  << " fps [" << fourcc_from_guid(actual_subtype) << "]\n";
        ...
        std::cout << "[3/3] Real-time capture thread running\n";
        ...
        while (g_running.load()) {
            ...
            if (elapsed >= 1.0) {
                const double capture_fps = static_cast<double>(frames_window) / elapsed;
                std::wostringstream status;
                status << L"MJPG 1280x480@60 -> YUY2 | capture "
                       << std::fixed << std::setprecision(1) << capture_fps
                       << L" fps | source " << std::setprecision(1) << source_hz
                       << L" Hz | frame " << frames_total;
                publish_frame(hwnd, local_frame, status.str());
                frames_window = 0;
                fps_epoch = now;
            } else {
                std::wostringstream status;
                status << L"MJPG 1280x480@60 -> YUY2 | measuring... | frame " << frames_total;
                publish_frame(hwnd, local_frame, status.str());
            }
        }
...
LRESULT CALLBACK window_proc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam) {
    ...
    case kFrameReadyMessage: {
        ...
        SetWindowTextW(hwnd, (L"TouchPlus Revival - Stereo Viewer RT - " + g_display_status).c_str());
        InvalidateRect(hwnd, nullptr, FALSE);
        return 0;
    }
    case kCaptureErrorMessage:
        MessageBoxA(hwnd, g_capture_error.c_str(), "TouchPlus Stereo Viewer RT", MB_OK | MB_ICONERROR);
        DestroyWindow(hwnd);
        return 0;
    case WM_CLOSE:
...
HWND create_viewer_window(HINSTANCE instance) {
    ...
    HWND hwnd = CreateWindowExW(
        0,
        kWindowClass,
        L"TouchPlus Revival - Stereo Viewer RT",
...
int wmain(int argc, wchar_t** argv) {
    ...
    try {
        std::cout << "TouchPlus Revival - Phase 1A real-time stereo viewer\n";
        std::cout << "Process architecture: " << (sizeof(void*) * 8) << "-bit\n";
        std::cout << "Pipeline: Etron unlock -> native MJPEG60 -> YUY2 -> custom BGRA\n";
        std::cout << "Orientation: historical Ractiv vertical flip before stereo split\n\n";
        ...
        std::cout << "Stereo viewer closed cleanly.\n";
        return 0;
    } catch (const std::exception& error) {
        std::cerr << "\nPHASE 1A RT RESULT: FAIL\n" << error.what() << "\n";
        MessageBoxA(nullptr, error.what(), "TouchPlus Stereo Viewer RT", MB_OK | MB_ICONERROR);
        return 1;
    }
}
