...
public:
    ...
    ContactResultV1 update(const ContactSampleV1& sample) {
        if (!sample.valid || sample.identity_id == 0 ||
            ...
            return fail_safe_release("upstream-invalid");
        }

        if (!have_previous_) {
            adopt_identity(sample);
            return make_result(ContactStateV1::Hover, ContactEventV1::None, sample, 0.0, 0.0, "identity-acquired");
        }

        if (sample.identity_id != identity_id_) {
            if (is_contact_active()) {
                const auto out = make_result(ContactStateV1::TouchUp, ContactEventV1::Up, previous_, 0.0, 0.0, "identity-switch");
                hard_reset();
                return out;
            }
            adopt_identity(sample);
            return make_result(ContactStateV1::Hover, ContactEventV1::None, sample, 0.0, 0.0, "identity-switch-reset");
        }
        ...
        switch (state_) {
            case ContactStateV1::NoFinger:
                state_ = ContactStateV1::Hover;
                result = make_result(state_, ContactEventV1::None, sample, h_velocity, xy_delta, "finger-valid");
                break;
            ...
            case ContactStateV1::Hover:
                ...
                if (sample.h_mm <= config_.approach_band_mm && approach_memory_ > 0) {
                    ...
                    result = make_result(state_, ContactEventV1::None, sample, h_velocity, xy_delta, "approach-evidence");
                } else {
                    result = make_result(state_, ContactEventV1::None, sample, h_velocity, xy_delta, "hover");
                }
                break;
            ...
            case ContactStateV1::Approaching:
                if (sample.h_mm > config_.approach_band_mm || approach_memory_ == 0) {
                    near_count_ = 0;
                    state_ = ContactStateV1::Hover;
                    result = make_result(state_, ContactEventV1::None, sample, h_velocity, xy_delta, "approach-lost");
                    break;
                }
                if (sample.h_mm <= config_.down_h_mm) {
                    if (xy_delta > config_.max_candidate_xy_step_mm) {
                        near_count_ = 0;
                        state_ = ContactStateV1::Hover;
                        result = make_result(state_, ContactEventV1::None, sample, h_velocity, xy_delta, "candidate-xy-reset");
                        break;
                    }
                    ++near_count_;
                    if (near_count_ >= std::max(1, config_.near_frames_required - 1)) {
                        state_ = ContactStateV1::ContactCandidate;
                        result = make_result(state_, ContactEventV1::None, sample, h_velocity, xy_delta, "near-surface-persistent");
                    } else {
                        result = make_result(state_, ContactEventV1::None, sample, h_velocity, xy_delta, "near-surface-counting");
                    }
                } else {
                    near_count_ = 0;
                    result = make_result(state_, ContactEventV1::None, sample, h_velocity, xy_delta, "approaching");
                }
                break;
            ...
            case ContactStateV1::ContactCandidate:
                if (sample.h_mm > config_.down_h_mm + config_.candidate_h_slack_mm || approach_memory_ == 0) {
                    ...
                    result = make_result(state_, ContactEventV1::None, sample, h_velocity, xy_delta, "candidate-aborted");
                    break;
                }
                if (xy_delta > config_.max_candidate_xy_step_mm) {
                    near_count_ = 0;
                ...
                if (near_count_ >= config_.near_frames_required) {
                    ...
                    held_ = true;
                    release_count_ = 0;
                    result = make_result(state_, ContactEventV1::Down, sample, h_velocity, xy_delta, "touch-confirmed");
                } else {
                    result = make_result(state_, ContactEventV1::None, sample, h_velocity, xy_delta, "contact-confirming");
                }
                break;
            ...
            case ContactStateV1::TouchHeld:
                if (sample.h_mm >= config_.release_h_mm) {
                    ...
                    if (release_count_ >= config_.release_frames_required) {
                        result = make_result(ContactStateV1::TouchUp, ContactEventV1::Up, sample, h_velocity, xy_delta, "release-hysteresis");
                        hard_reset();
                        return result;
                    }
                    result = make_result(ContactStateV1::TouchHeld, ContactEventV1::Held, sample, h_velocity, xy_delta, "release-counting");
                } else {
                    release_count_ = 0;
                    result = make_result(ContactStateV1::TouchHeld, ContactEventV1::Held, sample, h_velocity, xy_delta, "held");
                }
                break;
            ...
            case ContactStateV1::TouchUp:
                hard_reset();
                adopt_identity(sample);
                result = make_result(ContactStateV1::Hover, ContactEventV1::None, sample, h_velocity, xy_delta, "post-up-hover");
                break;
        }
...
private:
    ...
    ContactResultV1 fail_safe_release(const char* reason) {
        if (is_contact_active()) {
            ContactSampleV1 last = previous_;
            const auto out = make_result(ContactStateV1::TouchUp, ContactEventV1::Up, last, 0.0, 0.0, reason);
            hard_reset();
            return out;
        }
        hard_reset();
    ...
    ContactResultV1 make_result(ContactStateV1 state, ContactEventV1 event, const ContactSampleV1& sample,
                                double h_velocity, double xy_delta, const char* reason) const {
        ContactResultV1 out;
        out.state = state;
        out.event = event;
