...
$Probe = Join-Path $PSScriptRoot "touchplus_mediapipe_fusion_benchmark.py"
...
if (-not $Output) {
    $Output = Join-Path $StateRoot "output-fused"
}
...
if ($LASTEXITCODE -ne 0) { throw "2B.10M.1 fusion benchmark failed with exit code $LASTEXITCODE" }

Write-Host ""
Write-Host "2B.10M.1 fusion benchmark complete."
